  ...
  .addEventListener('submit', async (event) => {
    ...
    try {
      const data = await api('/api/templates/render', {
        ...
        body: JSON.stringify({
          title:
            document.querySelector('#templateTitle').value,
          template:
            document.querySelector('#templateBody').value
        })
      });

      // VULNERABLE VERSION (kept as a commented training reference):
      // The rendered value was inserted into innerHTML without encoding.
      //
      ...
      // SECURE VERSION:
      // User-controlled output is encoded before being inserted into HTML.
      ...
      showToast('Template rendered safely.');
    } catch (error) {
